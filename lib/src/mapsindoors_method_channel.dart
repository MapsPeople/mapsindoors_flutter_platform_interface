...
class MethodChannelMapsindoors extends MapsindoorsPlatform {
  ...
  void onPositionUpdate(MPPositionResultInterface position) {
    listenerChannel.invokeMethod(
        'MIL_onPositionUpdate', {"position": position._jsonEncode()});
  }

  @override
  Future<MPError?> load(String key) async {
    final error = await mapsIndoorsMethodChannel
        .invokeMethod('MIN_initialize', {"key": key});
  ...
  void addOnMapsIndoorsReadyListener(OnMapsIndoorsReadyListener listener) {
    if (_readyListeners.isEmpty) {
      listenerChannel.invokeMethod(
          "MIL_onMapsIndoorsReadyListener", {"addListener": true});
    }
  ...
  void removeOnMapsIndoorsReadyListener(OnMapsIndoorsReadyListener listener) {
    ...
    if (_readyListeners.isEmpty) {
      listenerChannel.invokeMethod(
          "MIL_onMapsIndoorsReadyListener", {"addListener": false});
    }
  }
  ...
  Future<void> disableEventLogging(bool disable) {
    return mapsIndoorsMethodChannel
        .invokeMethod("MIN_disableEventLogging", {"disable": disable});
  }
  ...
  Future<List<MPLocation>?> getLocationsByExternalIds(List<String> ids) async {
    final locations = await mapsIndoorsMethodChannel
        .invokeMethod("MIN_getLocationsByExternalIds", {"ids": ids});
    return convertMIList<MPLocation>(
        jsonDecode(locations), (p0) => MPLocation.fromJson(p0));
  }
  ...
  Future<MPSolution?> getSolution() async {
    final solution =
        await mapsIndoorsMethodChannel.invokeMethod("MIN_getSolution");
  ...
  Future<bool?> setLanguage(String language) {
    return mapsIndoorsMethodChannel
        .invokeMethod("MIN_setLanguage", {"language": language});
  }
  ...
  Future<MPGeocodeResult?> reverseGeoCode(MPPoint point) async {
    final geoCode = await mapsIndoorsMethodChannel
        .invokeMethod("MIN_reverseGeoCode", {"point": point._jsonEncode()});
  ...
  Future<MPVenue?> getDefaultVenue() async {
    final venue =
        await mapsIndoorsMethodChannel.invokeMethod("MIN_getDefaultVenue");
